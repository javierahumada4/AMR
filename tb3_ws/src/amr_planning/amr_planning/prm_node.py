...
class PRMNode(LifecycleNode):
    ...
    def on_configure(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        try:
            ...
            self._goal: Tuple[float, float] = tuple(
            ...
            self._smoothing_additional_points: int = (
                self.get_parameter("smoothing_additional_points")
                .get_parameter_value()
                .integer_value
            )
            self._smoothing_data_weight: float = (
                self.get_parameter("smoothing_data_weight")
                .get_parameter_value()
                .double_value
            )
            self._smoothing_smooth_weight: float = (
                self.get_parameter("smoothing_smooth_weight")
                .get_parameter_value()
                .double_value
            )
            use_grid: bool = (
                self.get_parameter("use_grid").get_parameter_value().bool_value
            )
            ...
            roadmap_creation_time: float = time.perf_counter() - start_time
    ...
    def _path_callback(self, pose_msg: AmrPoseStamped) -> None:
        ...
        if pose_msg.localized and not self._localized:
            ...
            path: List[Tuple[float, float]] = self._planning.find_path(
                start, self._goal
            )

            # Smooth the computed path using a weighted algorithm
            smoothed_path: List[Tuple[float, float]] = PRM.smooth_path(
                path=path,
