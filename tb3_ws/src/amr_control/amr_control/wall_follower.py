class WallFollower:
    ...
    def __init__(self, dt: float) -> None:
        ...
        self.state: str = "FORWARD"  # Initial state of the robot ("FORWARD" or "TURN")

        # PD controller gains for angular velocity control
        self.Kp: float = 80.0  # Proportional gain
        self.Kd: float = 40.0  # Derivative gain
        ...
        self.stop_distance: float = (
            0.25  # Minimum distance to stop before an obstacle [m]
        )
        self.follow_distance: float = 0.21  # Desired distance from the wall [m]

        # Velocity parameters
        self.v: float = 0.1  # Maximum linear velocity [m/s]
        self.w_control: float = (
            -0.1
        )  # Maximum angular velocity for wall following [rad/s]
        self.w_turn: float = -0.25  # Angular velocity for turning [rad/s]

        self.w_current: float = 0.0  # Current angular velocity during turning
    ...
    def compute_commands(
        ...
        front: list[float] = list(z_scan[0:15]) + list(z_scan[-15:])  # Front sector
        left: list[float] = list(z_scan[40:60])  # Left sector
        right: list[float] = list(z_scan[-60:-40])  # Right sector
        ...
        # State: FORWARD (Move forward while following a wall)
        if self.state == "FORWARD":
            ...
            # Transition to TURN state if an obstacle is detected in front
            if d_front < self.stop_distance:
                self.angle: float = 0.0
                self.state = "TURN"
                v = 0.0
                w = (
                    self.w_turn if d_left > d_right else -self.w_turn
                )  # Turn direction based on wall proximity
                self.w_current = w

        # State: TURN (Turn in place to avoid obstacles)
        elif self.state == "TURN":
            if (
                d_front > self.stop_distance + 0.05
            ):  # Transition back to FORWARD when front is clear
                self.state = "FORWARD"
                v = self.v
                w = (
                    -self.w_current * 0.5
                )  # Smoothly reduce angular velocity after turning
            else:
                v = 0.0
                w = self.w_current  # Maintain current angular velocity during turning
